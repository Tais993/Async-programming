...
is annoying, you have to constantly add it.
...
At least, you'd want to do that. But it's not that simple, threads aren't lightweight, threads are heavy. Threads
...
elaborate on this, if a thread calls interrupts another thread, this thread sets the status to Interrupted, and
...
Beautifully simple in the end, no major rewrites required. And this, while it's still really performant, if not even more than reactive libraries.
